...
void trocaValores(int *a, int *b, int *c, char trocas[], int total_char){
    ...
    for(int i = 0; i < total_char;i++){
        if((trocas[i] == 'A' && trocas[i + 1] == 'B') || (trocas[i] == 'B' && trocas[i+1] == 'A')){
            temp = *b;
            *b = *a;
            *a = temp;
        }else if ((trocas[i] == 'B' && trocas[i + 1] == 'C') || (trocas[i] == 'C' && trocas[i+1] == 'B')){
            temp = *c;
            *c = *b;
            *b = temp;
        }else if((trocas[i] == 'A' && trocas[i + 1] == 'C') || (trocas[i] == 'C' && trocas[i+1] == 'A')){
            temp = *c;
            *c = *a;
            *a = temp;
        }
